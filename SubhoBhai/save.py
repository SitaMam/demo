...
from SubhoBhai.strings import strings, HELP_TXT
...
async def downstatus(client: Client, statusfile, message):
    ...
    while os.path.exists(statusfile):
        with open(statusfile, "r") as downread:
            txt = downread.read()
        try:
            await client.edit_message_text(message.chat.id, message.id, f"⏳ Status Of Your Downloading... : {txt}")
...
async def upstatus(client: Client, statusfile, message):
    ...
    while os.path.exists(statusfile):
        with open(statusfile, "r") as upread:
            txt = upread.read()
        try:
            await client.edit_message_text(message.chat.id, message.id, f"⏳ Status Of Your Uploading... : {txt}")
            await asyncio.sleep(10)
        except:
            await asyncio.sleep(5)
...
async def send_start(client: Client, message: Message):
    buttons = [[
        InlineKeyboardButton('❣️ Developer', url='https://t.me/SubujDas')
    ],[
        InlineKeyboardButton('🤖 Bot Status', url='https://stats.uptimerobot.com/e9ecZVVuvL/')
    ]]
    ...
    await client.send_message(
        ...
        Welcome to ShieldedSaver Bot! 😊  
        I’m here to help you securely download restricted or limited-access content using its post link.  

        What You Can Do With ShieldedSaver:  
        - Download content from public or private chats.  
        - Handle multiple posts efficiently.  
        - Save posts with a single click!  

        To Get Started:  
        1️⃣ Type /login to authenticate yourself.  
        2️⃣ Use /help to learn about supported commands and detailed usage instructions.  

        Developer Note:  
        This bot is developed by @SubujDas for ethical purposes. Please use responsibly.  
        ...
        for msgid in range(fromID, toID+1):
	        ...
	        # public
                ...
                try:
                    ...
                    await client.send_message(message.chat.id, "ℹ️ The provided username does not exist or is not occupied by anyone. Please check and try again.", reply_to_message_id=message.id)
                    return
                try:
...
async def handle_private(client: Client, acc, message: Message, chatid: int, msgid: int):
    ...
    smsg = await client.send_message(message.chat.id, "❣️Please wait. Your Download Will Start Shortly...👀", reply_to_message_id=message.id)
